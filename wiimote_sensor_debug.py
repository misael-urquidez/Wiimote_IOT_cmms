...
class LinuxWiimote:
    ...
    def iniciar(self):
        ...
        print(f"\n[DEBUG] absinfo cargado: {self._absinfo}")
        print(f"[DEBUG] accel_dev.path={self.accel_dev.path} fd={self.accel_dev.fd}")
        t = threading.Thread(target=self._leer_acc, daemon=True)
        t.start()
        print(f"[DEBUG] hilo accel iniciado, alive={t.is_alive()}")
        try:
            self.btn_dev.grab()
        except Exception as e:
            print(f"[DEBUG] grab fallo: {e}")
    ...
    def _leer_acc(self):
        ...
        print("[DEBUG] _leer_acc: entrando a read_loop()")
        try:
            for ev in self.accel_dev.read_loop():
                if self._stop:
                    return
                print(f"[DEBUG] evento crudo: type={ev.type} code={ev.code} value={ev.value}")
